...
with tab1:
    ...
    reasonwhy = st.selectbox('Reason', list(dftodlers)[3:],key="orel")
    showbar(dftodlers, reasonwhy, '#0099C6')
    when = st.selectbox('Year', list(dftodlers['Year'].unique()),key="python")
...
with tab2:
    ...
    reasonwhy = st.selectbox('Reason', list(dfyoung)[3:],key="algosi")
    showbar(dfyoung, reasonwhy, 'plum')
    st.write('Stastistics of death on each year')
    when1 = st.selectbox('Year', list(dfyoung['Year'].unique()),key="matan")
    showpie(dfyoung, when1)
with tab3:
    st.header('How many people people of age between 15 and 49 died for different reasons')
    reasonwhy = st.selectbox('Reason', list(dfold)[3:],key="plusi]")
    showbar(dfold, reasonwhy, '#17BECF')
    st.write('Stastistics of death on each year')
    when2 = st.selectbox('Year', list(dfold['Year'].unique()),key="diskru")
    showpie(dfold, when2)
with tab4:
    st.header('How many people people of age between 50 and 69 died for different reasons')
    reasonwhy = st.selectbox('Reason', list(dfveryold)[3:],key="vse")
    showbar(dfveryold, reasonwhy, '#00CC96')
    st.write('Stastistics of death on each year')
    when3 = st.selectbox('Year', list(dfveryold['Year'].unique()),key="lynal")
